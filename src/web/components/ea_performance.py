"""EA Performance tab components.

Provides:
- EA summary table (overall, not date-range specific)
- Equity-style cumulative profit curve for selected EA or all
...
def _build_ea_summary(tx: pd.DataFrame):
    if tx.empty or "EA_Name" not in tx.columns:
        return html.Div("No EA data available.", className="text-muted")
    ...
    for ea, sub in tx.groupby("EA_Name"):
        ...
        # EA-specific equity curve from cumulative profit
    ...
    header = html.H5("EA Performance Summary (Overall)", className="mt-2 mb-2")
    return html.Div([header, _make_table(disp)])


def build_ea_performance_components(ea_filter: str):
    tx = _get_tx()
    if tx.empty:
        empty_fig = go.Figure()
    ...
    # Use only closed trades (if Result or similar exists), otherwise all
    # We leave this simple for now.

    # Summary table (all EAs)
    summary_table = _build_ea_summary(tx)

    # Filter for selected EA for charts & trade table
    tx_sel = tx
    if ea_filter and ea_filter != "all" and "EA_Name" in tx.columns:
        tx_sel = tx[tx["EA_Name"] == ea_filter].copy()
    ...
    equity_fig.update_layout(
        title="Cumulative Profit (Selected EA / All)",
    ...
    monthly_fig.update_layout(
        title="Monthly Profit (Selected EA / All)",
        template="plotly_white",
        xaxis_title="Month",
        yaxis_title="Profit",
        xaxis={"tickangle": -45},
    ...
    # Trades table
    if tx_sel.empty:
        trades_table = html.Div("No trades for selected EA.", className="text-muted")
    else:
        cols = []
